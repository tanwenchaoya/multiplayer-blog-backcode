...
router.post("/getlableInfo", (req, res) => {
  ...
  const sql = `SELECT * FROM article inner join user on lable = '${lable}' AND article.username = user.username`;
...
router.post("/getUserArticle", (req, res) => {
  ...
  jwt.verifyToken(token).then((data) => {
    mysql.connect(() => console.log("ok"));
    // const sql = `SELECT * FROM article WHERE lable = '${lable}';`;
    const sql = `SELECT * FROM article a inner join user on a.username = '${data.token.username}' AND a.username = user.username;`;
...
router.post("/getManycategroys", (req, res) => {
  ...
  const sql = `SELECT * FROM article inner join user on article_categroy = '${categroy}' AND article.username = user.username`;
...
router.post("/like_article_search", (req, res) => {
  ...
  //   "SELECT * FROM article WHERE LOCATE(?, title) > 0 ORDER BY id DESC;";
   const sql = "SELECT * FROM article a inner join user on LOCATE(?, title) > 0 AND a.username = user.username ORDER BY a.id DESC;";
  
  // const sql = `SELECT * FROM article a inner join user on lable = '${lable}' AND a.username = user.username`;
...
router.post("/articlePublish", (req, res) => {
  ...
  jwt
    ...
    .then((data) => {
      ...
      const sql =
        "INSERT INTO article(article_brief, article_img, lable, content, title, time, article_id, article_categroy,username) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);";
      const params = [
        brief,
        src,
        lable,
